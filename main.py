...
def build_manual_fields_keyboard(prospect_id: int) -> InlineKeyboardMarkup:
    keyboard = [
        [
            InlineKeyboardButton("✍️ Записати зараз", callback_data=f"manual_now:{prospect_id}"),
            InlineKeyboardButton("⏭ Пізніше", callback_data=f"manual_later:{prospect_id}"),
        ],
    ]
...
def build_after_research_keyboard(prospect_id: int) -> InlineKeyboardMarkup:
    keyboard = [
        [
            InlineKeyboardButton("✍️ Записати DR/Traffic/Price", callback_data=f"manual_now:{prospect_id}"),
        ],
        [
            InlineKeyboardButton("✉️ Email", callback_data=f"email:{prospect_id}"),
            InlineKeyboardButton("🧲 Subjects", callback_data=f"subjects:{prospect_id}"),
...
def parse_manual_fields_text(text: str) -> dict[str, str]:
    result = {}
    aliases = {
        "dr": "dr",
        "domain rating": "dr",
...
def manual_fields_help_text(prospect_id: int) -> str:
    return (
        f"Ок, запиши дані для prospect #{prospect_id} одним повідомленням у такому форматі:\n\n"
        "DR: 55\n"
        "Traffic: 12000\n"
        "Price: 80$\n"
        "Contact: Anna\n"
        ...
        "Niche: pets\n\n"
        "Можна заповнити не все, а тільки те, що знаєш."
    )
...
async def do_research(update: Update, prospect_id: int):
    ...
    await message.edit_text(
        ...
        + "\n\nХочеш зараз записати DR, Traffic, Price, Contact person, Email або Niche?",
        reply_markup=build_after_research_keyboard(prospect_id),
        disable_web_page_preview=True,
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    pending_manual_fields = context.user_data.get("pending_manual_fields")

    if pending_manual_fields:
        prospect_id = pending_manual_fields["prospect_id"]
        chat_id = update.effective_chat.id
        text = update.message.text.strip()

        if text.lower() in ["cancel", "скасувати", "відміна", "пізніше"]:
            context.user_data.pop("pending_manual_fields", None)
            await update.message.reply_text("Ок, заповниш пізніше ✅")
            return

        fields = parse_manual_fields_text(text)

        if not fields:
            await update.message.reply_text(
                "Не бачу полів у правильному форматі 😕\n\n"
                + manual_fields_help_text(prospect_id)
            )
            return
        ...
        try:
            for field, value in fields.items():
                update_manual_field(prospect_id, chat_id, field, value)
            ...
            await update.message.reply_text(
                "Дані збережено ✅\n\n"
                f"Оновлені поля: {', '.join(updated_fields)}\n\n"
                f"Подивитись картку: /view {prospect_id}"
            )
            return
        except Exception as error:
            logging.exception("Error while saving manual fields")
            await update.message.reply_text(
                "Не зміг зберегти дані 😕\n\n"
                f"Технічна помилка:\n{type(error).__name__}: {error}"
            )
            return
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if action == "manual_now":
            context.user_data["pending_manual_fields"] = {"prospect_id": prospect_id}
            await query.message.reply_text(manual_fields_help_text(prospect_id))
            return

        if action == "manual_later":
            await query.message.reply_text(
                "Ок, заповниш пізніше ✅\n\n"
                f"Коли буде потрібно, використовуй:\n"
                f"/set {prospect_id} dr 55\n"
                f"/set {prospect_id} traffic 12000\n"
                f"/set {prospect_id} price 80$\n"
                f"/set {prospect_id} contact Anna\n"
                f"/set {prospect_id} email hello@example.com"
            )
            return
